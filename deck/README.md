...
A 12-slide teaching deck for the July 16 "Build Your Second Brain in Two Hours" workshop (Ethan Davidson + Jeff Kazzee, produced by Joanna Kurylo). Non-technical audience. The concept is one sentence: your AI works better when it can read your files, here is the shape, here is how Zo builds it with you.

- Copy is the source of truth and lives in `../slides-copy.md`. It is final and voice-gated: sentence case, no em dashes, no banned words, defensive specificity, ranked positions. Cut for layout only, never paraphrase.
- The spec the deck teaches lives in `../spec.md`. Any file names, folder shapes, or code shown on screen must match it.
- Build prompt that produced this deck: `../build-deck.prompt.md`.

### Slide map

1. Title (type-led)
2. The hook (image-led, hands illustration)
3. What it is, the five things (image-led, five-things illustration + numbered list)
4. The one rule (image-led, sources vs current truth illustration)
5. The recipe, not the template (image-led, recipe-vs-template illustration + five questions)
6. Pick a depth (type-led, four-column typographic list)
7. How Zo reads your brain (image-led, index/network illustration)
8. Build anchor (type-led, terminal card with the five questions)
9. Ingest anchor (image-led, source-to-structure illustration)
10. Gates (type-led, six action pills)
...
Warm paper canvas, near-black ink, one accent. No SaaS fonts.

- Palette: `--d-canvas` warm off-white paper, `--d-ink` near-black, `--d-teal` / `--d-teal-dark` primary, `--d-coral` accent, `--d-slate` muted, `--d-line` hairline. Defined in `src/styles.css`. No gradients.
- Type: Space Grotesk (display + body) paired with JetBrains Mono (labels, eyebrows, counters, on-screen cues). Loaded via Google Fonts in `index.html`; system fallback offline.
- Layout: one full-screen `<section>` per slide on a 4px spacing scale. Chrome is fixed: quarter label top-right, slide counter + nav hint bottom-right, progress bar bottom-left. The quarter label sits top-right so it never collides with a left-aligned headline.
- Imagery: seven original flat vector illustrations in one visual language (orange-red + dark teal + white on cream, no text in images). Stored in `public/images/`. Generated with Nano Banana. The M.C. Escher "Drawing Hands" reference on slide 2 is an original ink-line take, not the copyrighted artwork.

### Navigation

ArrowRight and Space advance, ArrowLeft goes back. URL hash per slide (`#slide-1` through `#slide-12`), synced both ways via a `hashchange` listener and the key handler. Responsive but optimized for a projected large screen.

### Standalone export

A single self-contained HTML file lives at `../docs/index.html` (about 5.4 MB). Every image is inlined as a base64 data URI, and the JS and CSS are inlined, so it runs from any browser by double-clicking, offline, with no server. Keyboard nav and hash deep-links work identically to the Zo preview. Google Fonts load when online and fall back to system fonts offline.

The export is rebuilt with `bun run scripts/inliner.ts` after a `bunx vite build`. The inliner uses function-based replacements (not string replacements) so `$&` patterns inside the bundled JS are not reinterpreted, and escapes any literal `</script>` so the inlined module tag does not close early.

### Files added for this deck

- `src/deck/Deck.tsx` — the deck (12 slide components, nav, chrome, shared primitives).
- `src/styles.css` — deck design tokens appended to the base stylesheet.
- `public/images/slide-*.jpg` — the seven illustrations.
- `index.html` — Google Fonts links + deck title.
- `src/App.tsx` — routes `/` to the deck.
